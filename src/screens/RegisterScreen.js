...
async function registerUser(email, password) {
  ...
  if (registerResponse.status !== 201) {
    throw new Error(
      `${
        httpMessage[registerResponse.status][registerResponse.data.message]
  ...
  if (registerUserData) {
    ...
    if (loginResponse.status !== 200) {
      throw new Error(
        `${httpMessage[loginResponse.status][loginResponse.data.message]}` ||
          "Une erreur est survenue"
      );
    }
    ...
    if (loginDatas && loginDatas.status === 200) {
      ...
      throw new Error(
        `${httpMessage[loginDatas.status][loginDatas.message]}` ||
          "Une erreur est survenue"
      );
    }
  } else {
    throw new Error(
      `${httpMessage[registerResponse.status][registerResponse.message]}` ||
        "Une erreur est survenue"
    );
  }
}
