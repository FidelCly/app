...
<<<<<<< Updated upstream
	...
	const [userNotPromotionsLoader, setUserNotPromotionsLoader] = useState([true]);
	...
	const setDatas = async () => {
		if (user) {
			try {
				...
				setNotUserPromotions(notUserPromotions.filter((promo) => promo.isActive));
					...
					<View style={styles.leftColumn}>
						{userLoader ? (
							<View>
								...
								<ActivityIndicator size={"large"} animating={userLoader} color={MD2Colors.green200} />
							</View>
						) : user ? (
							<View>
								<View>
  ...
  const setDatas = async () => {
    if (user) {
      try {
        ...
        setUserPromotions(userPromotions.filter((promo) => promo.isActive));
        setShuffledUserPromotions(
          shuffleArray(userPromotions.filter((promo) => promo.isActive))
        );
        setUserPromotionsLoader(false);
        for (let i = 0; i < notUserCards.length; i++) {
          const shopPromotions = await getShopPromotions(notUserCards[i].id);
        ...
        setShuffledNotUserPromotions(
          shuffleArray(notUserPromotions.filter((promo) => promo.isActive))
        );
        setUserNotPromotionsLoader(false);
                      ...
                      <View style={styles.alignColumn}>
                        ...
                        <View style={styles.secondColumn}>
                          <View>
                            <Text style={styles.promoDate}>
                              Du{" "}
                              {new Date(promo.startAt).toLocaleDateString("fr")}{" "}
                              {"\n"}
                              au{" "}
                              {new Date(promo.endAt).toLocaleDateString("fr")}
                            </Text>
                ...
                {shuffledNotUserPromotions.length > 0 ? (
                  shuffledNotUserPromotions.slice(0, 3).map((promo, index) => (
                    <Pressable
                      ...
                      key={index}
                      onPress={() => {
                        const shop = getShopByPromotionId(
                          promo.shopId,
                          promo.id
                        );
                        props.navigation.navigate("InfoShopToAdd", {
                  ...
                  <Text style={styles.promoDescription}>
                    Aucune promotion en cours chez les commerçants autour de
                    votre position
                  </Text>
                )}
              </ScrollView>
